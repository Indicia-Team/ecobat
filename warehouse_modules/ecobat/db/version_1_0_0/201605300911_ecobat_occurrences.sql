...
COMMENT ON COLUMN ecobat_occurrences.passes IS 'Total number of passes during the night for this taxon as provided in the input file.';
...
COMMENT ON COLUMN ecobat_occurrences.import_guid IS 'Unique identifier of the import event which added this record.';
COMMENT ON COLUMN ecobat_occurrences.autogenerated IS 'Set to true for rows which are automatically generated to sum the number of passes for higher taxa.';

CREATE INDEX ix_ecobat_occurrences_import_guid
  ON ecobat_occurrences (import_guid ASC NULLS LAST);